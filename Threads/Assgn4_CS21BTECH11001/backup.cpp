...
class Params {
    ...
    Params() {
        ...
        fs >> n >> m >> req_l >> cool_l >> ride_l >> k;
        fs.close();
        f = fopen("output.txt", "w");
        pass = fopen("pass.csv", "w");
        car = fopen("car.csv", "w");

        exponential_distribution<> x1(1/ride_l);
        ride_exp = x1;
        exponential_distribution<> x2(1/req_l);
        req_exp = x2;
        exponential_distribution<> x3(1/cool_l);
        cool_exp = x3;
    }
...
void car_init(Params * the, int i) {
    // Car * car = new Car();
    museum->cars[i]->ready();
    fprintf(the->f, "Car %d ready at %lf.\n", i, now());
