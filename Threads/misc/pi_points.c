...
void * thread_init(void * v) {
    ...
    sem_wait(s);
    in_circle += num;
    char logname[10] = "logs/";
    t_count++;
    sprintf(logname + 5, "%d", t_count);
    ...
    // this part can be optimised using gettid in linux
    fprintf(f, "Points in circle: ");
    for(int i = 0; i < n; i++) {
        if(p[i].in) fprintf(f, "(%lf, %lf) ", p[i].x, p[i].y);
    }
...
void collate() {
    system("cat logs/* > out.txt");
    system("cat telem.txt out.txt > output.txt");
    system("rm telem.txt out.txt");
    system("rm logs/*");
    system("rmdir logs");
    sem_unlink("/sync");
}
